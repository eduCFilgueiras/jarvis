...
<main><h1>Jarvis Control</h1><p>Painel local de observabilidade</p><div class="panel"><b>Status: </b><span id="status">idle</span><br><button onclick="send()">Enviar</button><button class="stop" onclick="stopRun()">Parar</button><textarea id="message" rows="3" placeholder="Mensagem para o Jarvis"></textarea></div><div class="grid"><div class="panel"><h3>Transcricao</h3><div id="transcript" class="value">-</div></div><div class="panel"><h3>Resposta</h3><div id="response" class="value">-</div></div></div><div class="panel"><h3>Eventos</h3><div id="events" class="events">-</div></div></main>
<script>async function send(){let m=document.getElementById('message').value.trim();if(!m)return;await fetch('/api/process',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:m})});document.getElementById('message').value='';refresh()}async function stopRun(){await fetch('/api/stop',{method:'POST'});refresh()}async function refresh(){let s=await (await fetch('/api/status')).json();for(let k of ['status','transcript','response','events'])document.getElementById(k).textContent=k==='events'?s[k].join('\n')||'-':s[k]||'-'}setInterval(refresh,700);refresh()</script>"""
